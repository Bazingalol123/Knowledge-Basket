...
namespace template.Server.Controllers
{
    ...
    public class QuestionsController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAnswers(int authUserId, int questionId)
        {
            if (authUserId <= 0 )
            {
        ...
        public async Task<IActionResult> CreateAnswer(int authUserId, int questionId, [FromBody] AnswerToShow newAnswer)
        {
            if (authUserId <= 0)
            {
                return Unauthorized("User is not authorized.");
        ...
        public async Task<IActionResult> UpdateImages(int authUserId, int id, [FromBody] string ImgName)
        {
            if(authUserId <= 0)
            {
                return Unauthorized("User is not authorized");
