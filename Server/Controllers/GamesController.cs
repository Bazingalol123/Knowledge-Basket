...
namespace template.Server.Controllers
{
    ...
    public class GamesController : ControllerBase
    {
        ...
        public async Task<IActionResult> AddGames(int authUserId, NewGameDTO gameToAdd)
        {
            if (authUserId > 0)
            {
                object newGameParam = new
                {
                    ...
                    GameCode = 0,
                    isPublished = false,
                    questionTime = gameToAdd.questionTime,
                    UserId = authUserId,
                    CanPublish = false
                };
        ...
        [HttpGet("{id}")]
        public async Task<IActionResult> GetGame(int authUserId, int id)
        {
            if (authUserId > 0)
            {
                object param = new
                ...
                string gameQuery = "SELECT GameName,questionTime FROM Games WHERE ID = @ID AND UserId = @UserId";
                var gameRecord = await _db.GetRecordsAsync<EditGameDTO>(gameQuery, param);

                if (gameRecord == null || !gameRecord.Any())
                {
                    return NotFound("Game not found or user unauthorized to access this game.");
                }
